...
class TopologyPreFilter(PreFilter):
    ...
    def top_promising_samples_for(
        ...
        worker_processes_inputs = [
            (
                len(samples_featuring_start_entity),
                start_entity,
                end_entity,
                samples_featuring_start_entity[i],
                verbose,
            )
    ...
    def analyze_sample(self, input_data):
        ...
        (
            sample_to_analyze_head,
            sample_to_analyze_relation,
            sample_to_analyze_tail,
        ) = sample_to_analyze
        ...
        if (
            sample_to_analyze_head == start_entity
            and sample_to_analyze_tail == end_entity
        ) or (
            sample_to_analyze_tail == start_entity
            and sample_to_analyze_head == end_entity
        ):
        ...
        initial_accretion_entity = (
            sample_to_analyze_tail
            if sample_to_analyze_head == start_entity
            else sample_to_analyze_head
        )
        next_step_incomplete_paths.append(
            ([sample_to_analyze], initial_accretion_entity)
        )
        ...
        while not terminate:
            ...
            for incomplete_path, accretion_entity in cur_step_incomplete_paths:
                samples_featuring_accretion_entity = self.entity_id_2_train_samples[
                    accretion_entity
                ]

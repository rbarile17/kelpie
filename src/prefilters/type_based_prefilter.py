    ...
    def __init__(self, model: Model, dataset: Dataset):
        ...
        for h, r, t in dataset.train_samples:
            if not h in self.entity_id_2_train_samples:
                ...
                self.entity_id_2_relation_vector[h] = np.zeros(
                    2 * self.dataset.num_relations
                )

            if not t in self.entity_id_2_train_samples:
                self.entity_id_2_train_samples[t] = [(h, r, t)]
                self.entity_id_2_relation_vector[t] = np.zeros(
                    2 * self.dataset.num_relations
                )
    ...
    def most_promising_samples_for(
        ...
        if perspective == "head":
            ...
            worker_processes_inputs = [
                (
                    len(samples_featuring_head),
                    sample_to_explain,
                    x,
                    perspective,
                    verbose,
                )
                for x in samples_featuring_head
            ]
            results = self.thread_pool.map(
                self._analyze_sample, worker_processes_inputs
            )
            ...
            for i in range(len(samples_featuring_head)):
                sample_featuring_head_2_promisingness[
                    samples_featuring_head[i]
                ] = results[i]

            samples_featuring_head_with_promisingness = sorted(
                sample_featuring_head_2_promisingness.items(),
                reverse=True,
                key=lambda x: x[1],
            )
            sorted_promising_samples = [
                x[0] for x in samples_featuring_head_with_promisingness
            ]
            ...
            worker_processes_inputs = [
                (
                    len(samples_featuring_tail),
                    sample_to_explain,
                    x,
                    perspective,
                    verbose,
                )
                for x in samples_featuring_tail
            ]
            results = self.thread_pool.map(
                self._analyze_sample, worker_processes_inputs
            )
            ...
            for i in range(len(samples_featuring_tail)):
                sample_featuring_tail_2_promisingness[
                    samples_featuring_tail[i]
                ] = results[i]

            samples_featuring_tail_with_promisingness = sorted(
                sample_featuring_tail_2_promisingness.items(),
                reverse=True,
                key=lambda x: x[1],
            )
            sorted_promising_samples = [
                x[0] for x in samples_featuring_tail_with_promisingness
            ]
    ...
    def _analyze_sample(self, input_data):
        ...
        if verbose:
            print(
                ...
                + str(i)
                + " on "
                + str(all_samples_number)
                + ": "
                + self.dataset.printable_sample(sample_to_analyze)
            )

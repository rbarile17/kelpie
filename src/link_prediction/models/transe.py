...
class TransE(Model):
    ...
    In training or evaluation, our TransE class requires triples to be passed as 2-dimensional np.arrays.
    ...
    def score_embeddings(
        ...
        """
        # NOTE: this method is extremely important, because apart from being called by the Transe score(triples) method
    ...
    def all_scores(self, triples: np.array):
        ...
        :param triples: a 2-dimensional numpy array containing the triples to score, one per row
        :return: a 2-dimensional numpy array with a row for each passed triple and a column for each possible tail;
                 in any position, it contains the score for the triple of that row, computed with the tail of that column
        """
...
class KelpieTransE(KelpieModel, TransE):
    ...
    def predict_triples(self, triples: np.array, original_mode: bool = False):
        """
        This method overrides the Model predict_triples method by adding the possibility to run predictions
        ...
        scores, ranks, predictions = TransE.predict_triples(self, direct_triples)

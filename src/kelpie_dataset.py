...
class KelpieDataset(Dataset):
    ...
    def __init__(self, dataset: Dataset, entity_id: int):
        super(KelpieDataset, self).__init__(
            name=dataset.name, separator=dataset.separator, load=False
        )

        if dataset.num_entities == -1:
            raise Exception(
                "The Dataset passed to initialize a KelpieDataset must be already loaded"
            )
        ...
        self.num_entities = (
            dataset.num_entities + 1
        )  # adding the Kelpie entity to the count
        ...
        )  # add the kelpie entity to the dataset; it is always the last one
        ...
        self.original_train_samples = self._extract_samples_with_entity(
            dataset.train_samples, self.original_entity_id
        )
        self.original_valid_samples = self._extract_samples_with_entity(
            dataset.valid_samples, self.original_entity_id
        )
        self.original_test_samples = self._extract_samples_with_entity(
            dataset.test_samples, self.original_entity_id
        )

        self.kelpie_train_samples = Dataset.replace_entity_in_samples(
            self.original_train_samples, self.original_entity_id, self.kelpie_entity_id
        )
        self.kelpie_valid_samples = Dataset.replace_entity_in_samples(
            self.original_valid_samples, self.original_entity_id, self.kelpie_entity_id
        )
        self.kelpie_test_samples = Dataset.replace_entity_in_samples(
            self.original_test_samples, self.original_entity_id, self.kelpie_entity_id
        )
        ...
        for i in range(all_kelpie_samples.shape[0]):
            ...
            self.to_filter[(tail_id, relation_id + self.num_direct_relations)].append(
                head_id
            )
            ...
            if i < len(self.kelpie_train_samples):
                ...
                self.train_to_filter[
                    (tail_id, relation_id + self.num_direct_relations)
                ].append(head_id)
    ...
    def add_training_samples(self, samples_to_add: numpy.array):
        ...
        for sample in samples_to_add:
            assert (
                self.original_entity_id == sample[0]
                or self.original_entity_id == sample[2]
            )
        ...
        kelpie_samples_to_add = Dataset.replace_entity_in_samples(
            samples_to_add,
        ...
        for cur_head, cur_rel, cur_tail in kelpie_samples_to_add:
            ...
            self.to_filter[(cur_tail, cur_rel + self.num_direct_relations)].append(
                cur_head
            )
            self.train_to_filter[(cur_head, cur_rel)].append(cur_tail)
            self.train_to_filter[
                (cur_tail, cur_rel + self.num_direct_relations)
            ].append(cur_head)
        ...
        self.kelpie_train_samples = numpy.vstack(
            (self.kelpie_train_samples, numpy.array(kelpie_samples_to_add))
        )
    ...
    def remove_training_samples(self, samples_to_remove: numpy.array):
        ...
        kelpie_train_samples_to_remove = Dataset.replace_entity_in_samples(
        ...
        for cur_head, cur_rel, cur_tail in kelpie_train_samples_to_remove:
            ...
            self.to_filter[(cur_tail, cur_rel + self.num_direct_relations)].remove(
                cur_head
            )
        ...
        kelpie_train_indices_to_remove = [
            self.kelpie_train_sample_2_index[x] for x in kelpie_train_samples_to_remove
        ]
        self.kelpie_train_samples = numpy.delete(
            self.kelpie_train_samples, kelpie_train_indices_to_remove, axis=0
        )
    ...
    def as_kelpie_sample(self, original_sample):
        ...
        return Dataset.replace_entity_in_sample(
    ...
    def _extract_samples_with_entity(samples, entity_id):
        return samples[
            numpy.where(
                numpy.logical_or(samples[:, 0] == entity_id, samples[:, 2] == entity_id)
            )
        ]

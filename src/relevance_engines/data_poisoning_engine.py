...
class DataPoisoningEngine(ExplanationEngine):
    ...
    def removal_relevance(
        ...
        if len(samples_to_remove) > 1:
            raise NotImplementedError(
                "Data Poisoning Engine only supports single sample removal"
            )
        ...
        (
            ...
            _,
        ) = self.model.predict_sample(numpy.array(sample_to_explain))
        ...
        gradient = self.get_gradient_for(
            sample=sample_to_explain, entity_to_explain=entity_to_explain_id
        )
        ...
        if self.model.is_minimizer():
            relevance = (
                -original_sample_to_remove_score
                + self.lambd * perturbed_sample_to_remove_score
            )
        else:
            relevance = (
                original_sample_to_remove_score
        ...
        return (
            relevance,
            sample_to_explain_direct_score,
            sample_to_explain_target_rank,
            original_sample_to_remove_score,
            perturbed_sample_to_remove_score,
        )
    ...
    def addition_relevance(
        ...
        (
            ...
            _,
        ) = self.model.predict_sample(numpy.array(sample_to_convert))
        ...
        gradient = self.get_gradient_for(
            sample=sample_to_convert, entity_to_explain=entity_to_explain_id
        )
        ...
        if self.model.is_minimizer():
            relevance = (
                original_sample_to_add_score
                - self.lambd * perturbed_sample_to_add_score
            )
        else:
            relevance = (
                -original_sample_to_add_score
        ...
        return (
            relevance,
            sample_to_convert_direct_score,
            sample_to_convert_target_rank,
            original_sample_to_add_score,
            perturbed_sample_to_add_score,
        )

    def get_gradient_for(self, sample: Tuple[Any, Any, Any], entity_to_explain: int):
        sample_head, sample_relation, sample_tail = sample
        assert entity_to_explain in ([sample_head, sample_tail])

        if (
            (sample_head, sample_relation, sample_tail),
            entity_to_explain,
        ) in self.gradients_cache:
            return self.gradients_cache[
                ((sample_head, sample_relation, sample_tail), entity_to_explain)
            ].cuda()
        ...
        sample_head_embedding = (
            self.model.entity_embeddings[sample_head]
            .detach()
            .reshape(1, entity_dimension)
        )
        sample_relation_embedding = (
            self.model.relation_embeddings[sample_relation]
            .detach()
            .reshape(1, relation_dimension)
        )
        sample_tail_embedding = (
            self.model.entity_embeddings[sample_tail]
            .detach()
            .reshape(1, entity_dimension)
        )
        ...
        cur_entity_to_explain_embedding = (
            sample_head_embedding
            if entity_to_explain == sample_head
            else sample_tail_embedding
        )
        cur_entity_to_explain_embedding.requires_grad = True

        current_score = self.model.score_embeddings(
            sample_head_embedding, sample_relation_embedding, sample_tail_embedding
        )

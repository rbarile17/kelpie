...
class CriageEngine(ExplanationEngine):
    ...
    def removal_relevance(
        ...
        if len(samples_to_remove) > 1:
            raise NotImplementedError(
                "Criage Engine only supports single sample removal"
            )
        ...
        if perspective == "tail":
            ...
            if tail_to_explain in self.tail_entity_to_train_samples:
                train_samples_with_tail_as_tail = self.tail_entity_to_train_samples[
                    tail_to_explain
                ]

            hessian_matrix = self.get_hessian_for(
                entity=tail_to_explain, train_samples=train_samples_with_tail_as_tail
            )
            ...
            score_variation, _ = self._estimate_score_variation_in_removal(
                z_sample_to_explain=z_head_relation,
        ...
        # this means that the tail of sample_to_remove is the head of the sample to explain
            ...
            if head_to_explain in self.tail_entity_to_train_samples:
                train_samples_with_head_as_tail = self.tail_entity_to_train_samples[
                    head_to_explain
                ]

            hessian_matrix = self.get_hessian_for(
                entity=head_to_explain, train_samples=train_samples_with_head_as_tail
            )

            z_cur_head_relation = self.get_z_for(sample_to_remove)
            score_variation, _ = self._estimate_score_variation_in_removal(
                z_sample_to_explain=z_tail_relation,
                z_sample_to_remove=z_cur_head_relation,
    ...
    def addition_relevance(
        ...
        if len(samples_to_add) > 1:
            raise NotImplementedError(
                "Criage Engine only supports single sample addition"
            )
        ...
        if perspective == "tail":
            ...
            hessian_matrix = self.get_hessian_for(
                entity=tail_to_convert, train_samples=train_samples_with_tail_as_tail
            )
        ...
        # this means that the tail of sample_to_add is the head_to_convert
            ...
            hessian_matrix = self.get_hessian_for(
                entity=head_to_convert, train_samples=train_samples_with_head_as_tail
            )
    ...
    def compute_hessian_for(self, entity, samples_featuring_entity_as_tail):
        """
    ...
    def _estimate_score_variation_in_addition(
        self, z_sample_to_explain, z_sample_to_add, entity_id, entity_hessian_matrix
    ):
        entity_embedding = (
        ...
        m = numpy.linalg.inv(
            ...
            + sig_tri
            * (1 - sig_tri)
            * numpy.dot(numpy.transpose(z_sample_to_add), z_sample_to_add)
        )
        relevance = numpy.dot(
            z_sample_to_explain,
    ...
    def _estimate_score_variation_in_removal(
        self, z_sample_to_explain, z_sample_to_remove, entity_id, entity_hessian_matrix
    ):
        entity_embedding = (
            self.model.entity_embeddings[entity_id].detach().cpu().numpy()
        )
        ...
        try:
            m = numpy.linalg.inv(
                ...
                + sig_tri
                * (1 - sig_tri)
                * numpy.dot(numpy.transpose(z_sample_to_remove), z_sample_to_remove)
            )
            relevance = numpy.dot(
                z_sample_to_explain,
        ...
        with torch.no_grad():
            ...
            if perspective == "head":
                ...
                for cur_entity in range(0, dataset.num_entities):
                    ...
                    if (
                        degree_cap != -1
                        and dataset.entity_2_degree[cur_entity] > degree_cap
                    ):
                        continue
                    ...
                    if (cur_entity, relation_to_explain) in dataset.to_filter:
                        ...
                        if dataset.relation_2_type[relation_to_explain] in [
                            ONE_TO_ONE,
                            MANY_TO_ONE,
                        ]:
                            continue
                ...
                while batch_start < len(step_1_samples):
                    cur_batch = step_1_samples[
                        batch_start : min(len(step_1_samples), batch_start + batch_size)
                    ]
                    cur_batch_all_scores = (
                        model.all_scores(samples=numpy.array(cur_batch))
                        .detach()
                        .cpu()
                        .numpy()
                    )
                ...
                for i in range(len(step_1_candidate_entities)):
                    ...
                    if model.is_minimizer():
                        ...
                        cur_sample_target_score_filtered = cur_sample_all_scores[
                            cur_tail
                        ]
                        if (
                            1e6
                            > cur_sample_target_score_filtered
                            > numpy.min(cur_sample_all_scores)
                        ):
                        ...
                        if (
                            -1e6
                            < cur_sample_target_score_filtered
                            < numpy.max(cur_sample_all_scores)
                        ):
                            overall_candidate_entities.append(cur_candidate_entity)
